...
def init_weights(m):
    ...
    if classname.find('Conv2d') != -1 or classname.find('ConvTranspose2d') != -1:
        mindspore.common.initializer.HeUniform(m.weight)
        mindspore.common.initializer.Zero(m.bias)
    elif classname.find('BatchNorm') != -1:
        #mindspore.common.initializer.Normal(m.weight, 1.0, 0.02)
        initializer(Normal(1.0, 0.02), m.weight, mindspore.float32)
        mindspore.common.initializer.Zero(m.bias)
    elif classname.find('Linear') != -1:
        #nn.init.xavier_normal_(m.weight)   替代
        mindspore.common.initializer.XavierUniform(m.weight)
        mindspore.common.initializer.Zero(m.bias)
...
class RandomLayer(nn.Cell):
    def __init__(self, input_dim_list=[], output_dim=1024):
        ...
        self.random_matrix = [mindspore.ops.StandardNormal(input_dim_list[i], output_dim) for i in range(self.input_num)]

    def forward(self, input_list):
        return_list = [mindspore.ops.MatMul(input_list[i], self.random_matrix[i]) for i in range(self.input_num)]
        return_tensor = return_list[0] / math.pow(float(self.output_dim), 1.0/len(return_list))
        for single in return_list[1:]:
            return_tensor = mindspore.ops.Mul(return_tensor, single)
    ...
    def __init__(self, local_size=1, alpha=1.0, beta=0.75, ACROSS_CHANNELS=True):
        ...
        if ACROSS_CHANNELS:
            self.average=mindspore.ops.AvgPool3D(kernel_size=(local_size, 1, 1),
                    strides =1,
                    pad =(int((local_size-1.0)/2), 0, 0,int((local_size-1.0)/2), 0, 0)) #填充
        else:
            self.average=nn.AvgPool2d(kernel_size=local_size,
                    stride=1,
                    #padding=int((local_size-1.0)/2)) #无填充选项
                                      )
        self.alpha = alpha
        self.beta = beta
...
class AlexNet(nn.Cell):
    ...
    def __init__(self, num_classes=1000):
        ...
        self.features = nn.SequentialCell(
            nn.Conv2d(3, 96, kernel_size=11, stride=4, padding=0, has_bias=True),
            nn.ReLU(),
            LRN(local_size=5, alpha=0.0001, beta=0.75),
            nn.MaxPool2d(kernel_size=3, stride=2),
            nn.Conv2d(96, 256, kernel_size=5, padding=2, groups=2, has_bias=True),
            nn.ReLU(),
            LRN(local_size=5, alpha=0.0001, beta=0.75),
            nn.MaxPool2d(kernel_size=3, stride=2),
            nn.Conv2d(256, 384, kernel_size=3, padding=1, has_bias=True),
            nn.ReLU(),
            nn.Conv2d(384, 384, kernel_size=3, padding=1, groups=2, has_bias=True),
            nn.ReLU(),
            nn.Conv2d(384, 256, kernel_size=3, padding=1, groups=2, has_bias=True),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=3, stride=2),
        )
  ...
  def __init__(self, use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=1000):
    ...
    if new_cls:
        if self.use_bottleneck:
            self.bottleneck = nn.Dense(4096, bottleneck_dim)
            self.fc = nn.Dense(bottleneck_dim, class_num)
            self.bottleneck.apply(init_weights)
            self.fc.apply(init_weights)
            self.__in_features = bottleneck_dim
        else:
            self.fc = nn.Dense(4096, class_num)
            self.fc.apply(init_weights)
...
class ResNetFc(nn.Cell):
  def __init__(self, resnet_name, use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=1000):
    ...
    self.feature_layers = nn.SequentialCell(self.conv1, self.bn1, self.relu, self.maxpool, \
    ...
    if new_cls:
        if self.use_bottleneck:
            self.bottleneck = nn.Dense(model_resnet.fc.in_features, bottleneck_dim)
            self.fc = nn.Dense(bottleneck_dim, class_num)
            self.bottleneck.apply(init_weights)
            self.fc.apply(init_weights)
            self.__in_features = bottleneck_dim
        else:
            self.fc = nn.Dense(model_resnet.fc.in_features, class_num)
            self.fc.apply(init_weights)
...
class LeNet(nn.Cell):
    def __init__(self):
        ...
        self.fc_params = nn.SequentialCell(nn.Dense(50*4*4, 500), nn.ReLU(), nn.Dropout(keep_prob=0.5))
        self.classifier = nn.Dense(500, 10)
